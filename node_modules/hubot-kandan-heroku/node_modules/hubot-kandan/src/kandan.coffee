...
# Node API
HTTP         = if process.env.HUBOT_KANDAN_PROTOCOL == "https" then require("https") else require("http")
...
class Kandan extends Adapter
  ...
  run: ->
    ...
    @bot.on "TextMessage", (message) ->
      ignoredUsers = process.env.HUBOT_KANDAN_IGNORED_USERS or ""

      unless message.user.username in ignoredUsers.split(',')
